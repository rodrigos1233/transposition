...
describe('circleOfFifthModeShifter', () => {
  ...
  it('shifts correctly for Minor mode', () => {
    ...
    expect(circleOfFifthModeShifter(1, 0)).toBe((0 + minorPosition + 12) % 12);
    expect(circleOfFifthModeShifter(1, 5)).toBe((5 + minorPosition + 12) % 12);
  });

  it('shifts correctly for Dorian mode', () => {
    const dorianPosition = MODES[2].modePosition;
    expect(circleOfFifthModeShifter(2, 0)).toBe((0 + dorianPosition + 12) % 12);
    expect(circleOfFifthModeShifter(2, 5)).toBe((5 + dorianPosition + 12) % 12);
  });
});
