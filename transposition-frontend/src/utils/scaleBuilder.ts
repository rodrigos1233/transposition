  ...
  return possibleStartNotes.map((startNote) => {
    ...
    const alterationFromStartNote = startNoteName.includes('♭')
      ...
      : startNoteName.includes('♯')
        ? 'sharp'
        : undefined;
    return keySignatureBuilder(startNote, modeIndex, alterationFromStartNote);
  });
}
    ...
    if (!!alteration) {
      if (
        ...
        if (doubleAlteredNotes.includes(reducedNote)) {
          note.english = `${note.english}${alteration === 'flat' ? '𝄫' : '𝄪'}`;
          note.romance = `${note.romance}${alteration === 'flat' ? '𝄫' : '𝄪'}`;
          ...
          note.english = `${note.english}${alteration === 'flat' ? '♭' : '♯'}`;
          note.romance = `${note.romance}${alteration === 'flat' ? '♭' : '♯'}`;
          note.german = `${note.german}${alteration === 'flat' ? 'es' : 'is'}`;
        }
      }
