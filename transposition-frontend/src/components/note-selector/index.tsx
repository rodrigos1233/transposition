...
function NoteSelector(
    ...
    return (
        <div className={`note-selector ${isMobile ? 'note-selector--mobile' : ''}`}>
            {
                selectedNotes.map((note, k) => {
                    const isBlackNote = note.english.includes('♭') || note.english.includes('♯');

                    const nextIsBlackNote =
                        k + 1 < selectedNotes.length &&
                        (selectedNotes[k + 1].english.includes('♭') || selectedNotes[k + 1].english.includes('♯'));
                    ...
                    if (k > 0 && selectedNotes[k].english.includes('♭') && blackNotesAreHalf) {
                        return null;
                    }
                    ...
                    return (
                        <Button
                            ...
                            onClick={() => setSelected(k)}
                            disabled={k === selected}
                            className={isBlackNote ? 'bg-neutral-800 text-white' : 'bg-neutral-100'}
